...
Expected: **126 tests passing**, 0 failures. These cover:
...
- The demo video quotes the suite total measured on 2026-08-30. On 2026-08-31 a guard landed in `engine/__tests__/isolation.test.ts` asserting that nothing under `engine/` imports an external package, calls `fetch` or reads `process.env`, so the regulatory verdict stays structurally unreachable by any model. That raised the total, and `docs/FACTS.json` carries the current figure. A published video cannot be re-cut, so the difference is stated here rather than left for a judge to find: the count grew, no claim changed, and nothing was edited downward to match the recording.
