...
| **Vector file format** | Tylin | `app/api/ask/route.ts` | `corpus/vectors.npy` is a numpy binary and TypeScript cannot read it. Decided: the pipeline emits **`corpus/vectors.f32`**, a raw little-endian `Float32Array` with dimensions recorded in `corpus/schema.json`, loadable directly in a route handler. **Q6 DECIDED 2026-08-21:** Built in CI, stored in Vercel Blob. The corpus-build workflow (.github/workflows/corpus-build.yml) embeds all chunks and uploads manifest.sqlite, vectors.f32, and schema.json to Vercel Blob. The route handler fetches from Blob on cold start and caches in memory. corpus/chunks/ (JSON text only) is committed to git. No binary artifacts in git. |
