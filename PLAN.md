...
503 in production. Root cause is now positively verified, not guessed: the route
loads the corpus from Vercel Blob (`app/api/ask/route.ts` reads
`BLOB_READ_WRITE_TOKEN`, then lists the `corpus/` prefix), and the only thing that
ever uploads that bundle is `corpus-build.yml`, which is `workflow_dispatch` only
and **has never run once** (`gh run list --workflow=corpus-build.yml` returns zero
runs, exit 0). It could not have succeeded anyway, because **the repository has no
Actions secrets at all** (`gh secret list` returns empty, exit 0), so
`BLOB_READ_WRITE_TOKEN` and the `WATSONX_*` trio are absent. This is task 1.18,
and it now blocks 4.3, 4.4 and the Technical Execution criterion. Unblocking is two
steps: provision `BLOB_READ_WRITE_TOKEN` as a repo secret, then dispatch
`corpus-build.yml` with `hash_embeddings=true` (no watsonx spend). The bundle
builds locally today at 3.5 MB sqlite plus 10.3 MB vectors over 3,524 chunks.
