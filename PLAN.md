...
| 1.15 | Validator outreach sent | local only, never committed | **Stephen** | ⬜ | n/a | See "Real users" below. **Send on day zero (Aug 15), not day one.** August is the worst month for faculty reply. |
...
| # | Component | File(s) | Owner | Status | Deps | Notes |
|---|---|---|---|---|---|---|
| 2.1 | Mission setup flow, persists to IndexedDB only | `app/mission/**`, `lib/store.ts` | **Khadim** | ⬜ | 1.7 | Launch and delivery date, LV determination date, integration date, pathway (amateur vs experimental), frequencies, imaging yes/no, orbit apogee and perigee. Nothing transmitted server-side except push subscriptions. |
| 2.2 | Dependency graph view, violated and at-risk visually distinct | `components/graph/**` | **Khadim** | ⬜ | 1.7, 1.14 | `@xyflow/react` + `@dagrejs/dagre`. Run layout client-side after node measurement. |
...
| 2.7 | Deorbit compliance panel, the innovation surface | `components/deorbit/**` | **Khadim** | ⬜ | 1.12 | Shows the verdict, the lifetime estimate, the F10.7 value that produced it, the uncertainty band from NOAA's envelope, and the citation. Plus the swing: what the same orbit does at solar min. |
...
| 2.9 | Deadline banner, primary alert, zero permissions | `components/deadline-banner/**` | **Khadim** | ⬜ | 1.7 | Computed on load. This is what judges actually see. It must work with no notification permission granted. |
...
| 2.17 | `/api/status`, unauthenticated, recomputes the headline **and self-reports the wiring** | `app/api/status/route.ts` | **Stephen** | ✅ | 1.7, 2.16 | Recomputes the number on every request from the seeded mission. No key. This is the judge's proof. **Also returns the model IDs actually invoked in this deployment** (`generation`, `audit`, `embedding`, `surya` or `null` if cut) so claimed-versus-invoked is checkable by one curl. The endpoint must self-report what the README says, and CI asserts the two match. |
| 2.18 | `docs/FACTS.json` generator + the test that enforces it | `scripts/facts.py`, `tests/test_no_fabricated_numbers.py` | **Stephen** | ✅ | 2.17 | One real run writes every number. README, video narration and submission all read from it. |
| 2.19 | **iOS build uploaded to App Store Connect** | n/a | **Stephen** | ⬜ | 2.12, 2.13, 2.14 | Bundle ID registered, `ITSAppUsesNonExemptEncryption` NO, `PrivacyInfo.xcprivacy` present (missing it means an ITMS-91053 upload rejection), icons and splash generated. Internal testing confirms it launches. |
| 2.20 | **First EXTERNAL TestFlight build submitted for Beta App Review** | n/a | **Stephen** | ⬜ | 2.19 | **Target Aug 22, no later than Aug 23.** Beta app description, contact info, and a reviewer note naming each native feature and exactly how to reach it. |
| 2.21 | Signed Android APK/AAB + Firebase App Distribution invite link | `android/**` | **Stephen** | ⬜ | 0.11, 2.12 | Free, invite links work without a Google group, 500 testers. **Do not attempt a Play Store listing**, the 12-tester 14-day closed-test rule makes it impossible inside the window. |
...
| 3.3 | `/judge` page: numbered three-minute itinerary with deep links | `app/judge/page.tsx` | **Khadim** | ⬜ | 2.17 | Build the judge's door. Every claim reachable without logging in, without a key, without running anything. Seeded so an empty state is impossible. |
...
| 3.12 | Validator call taken, sanitized quote landed | `README.md` | **Stephen** | ⬜ | 1.15 | Substance in, attribution out. No names, no emails, no institutions without per-surface consent. |
| 3.13 | Re-read the BeMyApp announcements feed | n/a | **Stephen** | ⬜ | 0.15 | Organizer announcements are scoring intent and they arrive after you freeze. |
...
**Beneficiary sizing, in the submission.** **Every figure below is currently second-hand from the research pack and must be re-verified to a primary source with a URL and a year before it appears on any public surface**, per the rule that a number promoted into a claim must be sourced: roughly 40 or more university-class missions per year (Swartwout CubeSat Database, get the URL and the edition year); roughly 40% of university-class missions fail to achieve primary objectives with university failure events outnumbering professional-class by about 23 to 8 (Swartwout / AIAA-USU, get the paper); and NASA's state-of-the-art smallsat guidance that teams typically need a full year for the complete licensing process (get the report edition and page). **Owner: Stephen, due before 3.4.** Any figure that does not resolve gets cut rather than softened. Manifest is for the teams who did not start early.
