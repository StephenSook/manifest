...
| 1.13 | Bob evidence: Plan-mode output committed, screenshot #2 | `docs/bob-evidence/plan-mode-critical-path.md`, `docs/bob-evidence/bobalytics-02.png` | **Khadim** | 🟡 | 0.3 | Plan IS a real Bob 2.0.3 built-in, so this row stands as written, unlike 2.22 which named a mode that does not exist. Done means both files exist: the full Plan-mode transcript for the build's critical path, and the week-2 screenshot. `bobalytics-02.png` is committed. |
...
| 2.22 | Bobalytics screenshot #3, LANE-ENFORCEMENT transcript captured | `docs/bob-evidence/bobalytics-03.png`, `docs/bob-evidence/lane-enforcement.md` | **Khadim** | 🟡 | n/a | **REWRITTEN 2026-08-26: Bob 2.0.3 HAS NO ORCHESTRATOR MODE.** This row and 1.13 were written against a version that had one. Verified by grep over `/Applications/IBM Bob.app`: the only `orchestrator` matches in the entire bundle are inside TypeScript's own compiler files. The switcher offers Agent, Plan and Ask plus our five workspace modes. Khadim found this while trying to produce the artifact. **The replacement is stronger evidence, not a downgrade.** A transcript of a write-scoped mode REFUSING an out-of-lane write, citing its own fileRegex and naming the mode to switch to, demonstrates lane isolation happening rather than claimed. Khadim hit this for real: Frontend mode refused to write `docs/architecture.svg` and he had to switch to `evidence-writer`. Only 4 of 46 rivals have artifact-grade Bob evidence at all, and none demonstrates configured enforcement. `bobalytics-03.png` landed in `e409a0d`; the transcript is what remains. |
