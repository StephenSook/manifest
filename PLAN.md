...
| 0.8 | Next.js 15 + TS strict + Tailwind v4 + shadcn new-york scaffold | `app/**`, `components/**`, `package.json` | **Khadim** | ✅ | 0.2 | Done 2026-08-22. Next.js 16.3.1, TS strict, Tailwind v4 theme tokens. Deps pinned: `@dagrejs/dagre` 1.1.4 (scoped). `vis-timeline` and TanStack Table were later removed (`2b46358`); timeline task 2.3 is cut. No `elkjs`. `test:engine` and `test:e2e` added. shadcn init deliberately NOT run: pages are hand-styled to the dense regulatory-instrument spec, shadcn would write competing CSS variables into `globals.css`, and skipping it keeps the tree smaller for the 3.8 license audit. `postcss.config.mjs` was required: Tailwind v4 does not compile `@theme` without `@tailwindcss/postcss`. |
...
| 3.5 | Architecture diagram as a designed artifact | `docs/architecture.svg` | **Khadim** | ✅ | n/a | Done 2026-08-25. docs/architecture.svg written in evidence-writer mode. Edge labels carry the invariant. Node count is conditional on pathway (1569a6c). Mermaid source handed to Stephen for the README. |
...
- [x] **Q13 (Stephen, resolved 2026-08-26, Khadim backup):** Steal 25. Tasks 1.13 and 2.22 close as honesty logs, not invented transcripts. Plan-mode session never captured. Lane enforcement is yaml + git + CI. `docs/bob-evidence/**` is Khadim's lane; backup taken. Commits `8cce41b`, `b3ed2c8`, `b07e46d`.
