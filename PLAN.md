...
- [ ] **Q10 (Khadim, 2026-08-24 ping):** Task 1.18 must provision `WATSONX_API_KEY`, `WATSONX_PROJECT_ID`, `WATSONX_REGION=us-south`, `BLOB_READ_WRITE_TOKEN` on Vercel and as GitHub Actions secrets before `/api/ask` can run Granite+Guardian on the deployed URL and before corpus-build can upload. Task 1.17 must set repo variable `MANIFEST_DEPLOY_URL` (not a secret) or the uptime workflow fails honestly. After the variable is set, ping Tylin: dispatch `uptime.yml` and require the run `success` via check-runs (never `--watch`). Uptime now also POSTs `/api/ask` for a `97.207(g)(1)` citation. Tylin's gather list (no values) is in `TYLIN_TASKS.md` under "External credentials to gather".
...
_Last updated: 2026-08-24 by Tylin. 3.6 mid-phase re-grep. 3.8 gitleaks proven on 33a1800 plus license-guard. 3.10 /api/ask corpus check. Freeze 3.6 re-run waits on 3.7 (Q12). Uptime green waits on Khadim 1.17 (Q10)._
