...
| 2.20 | **First EXTERNAL TestFlight build submitted for Beta App Review** | n/a | **Stephen** | ✅ **APPROVED by Apple.** Verified 2026-08-25 via the ASC API (`GET /v1/builds/{id}/betaAppReviewSubmission`): app `Manifest CubeSat Licensing` id 6804876175, build 1, processingState VALID, expired false, betaReviewState **APPROVED**, submitted 2026-08-24T17:07 PT. Build is attached to the External Testers group. **ONE STEP REMAINS and it is outward-facing:** that group has `publicLinkEnabled: false`, so no public join URL exists yet. Enabling it is a single `PATCH /v1/betaGroups/{id}` with `publicLinkEnabled: true`, after which the URL fills the `[TESTFLIGHT-URL]` placeholder in `docs/submission.md` and the iOS row in the README. Held pending Stephen's go-ahead because it publishes a publicly joinable beta. | 2.19 | **Target Aug 22, no later than Aug 23.** Beta app description, contact info, and a reviewer note naming each native feature and exactly how to reach it. |
