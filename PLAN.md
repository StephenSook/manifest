...
|---|---|---|---|---|---|---|
| 0.1 | Copy this plan into the repo as `PLAN.md`, commit first | `PLAN.md` | **Stephen** | ⬜ | n/a | This is commit one. Repo public from here. |
| 0.2 | `git init`, public repo `StephenSook/manifest`, MIT license, README stub | `LICENSE`, `README.md`, `.gitignore` | **Stephen** | ⬜ | 0.1 | Keep the existing `.gitignore`. Verify `git ls-files \| grep -i '\.pdf$'` returns zero before any push. |
| 0.3 | Bob `/init` run, `.bob/` scaffold committed | `.bob/` except `skills/`, `AGENTS.md` | **Stephen** | ⬜ | 0.2 | See "Bob setup" below. `.bob/` is never gitignored. **Do not create `.bob/skills/`, that is Tylin's in 0.5.** |
| 0.4 | **Five** Bob custom modes with fileRegex write scopes | `.bob/custom_modes.yaml` | **Stephen** | ⬜ | 0.3 | Regexes are given verbatim under "Bob setup". Keys are `slug`, `name`, `description`, `roleDefinition`, `whenToUse`, `customInstructions`, `groups`. Pattern from `~/dev/IBM July/.bob/custom_modes.yaml`. **Test each mode by opening a file it should refuse.** |
...
| 0.6 | Workspace `.bob/mcp.json`, **no secrets** | `.bob/mcp.json` | **Stephen** | ⬜ | 0.3 | Bob cannot expand `${VAR}`, it stores values literally. Only the local eval MCP server goes here. Keyed servers stay in `~/.bob/mcp.json`. |
| 0.7 | Add missing MCP servers to Bob global config | `~/.bob/mcp.json` (local, not committed) | **Stephen** | ⬜ | n/a | Bob is missing `huggingface` (needed for Surya), `deepwiki`, `aws-docs`. Add those three. |
| 0.8 | Next.js 15 + TS strict + Tailwind v4 + shadcn new-york scaffold | `app/**`, `components/**`, `package.json` | **Khadim** | ✅ | 0.2 | Done 2026-08-22. Next.js 16.3.1, TS strict, Tailwind v4 theme tokens. Deps pinned: `@dagrejs/dagre` 1.1.4 (scoped), `vis-timeline` 8.5.4, TanStack Table 8.21.3. No `elkjs`. `test:engine` and `test:e2e` added. shadcn init deliberately NOT run: pages are hand-styled to the dense regulatory-instrument spec, shadcn would write competing CSS variables into `globals.css`, and skipping it keeps the tree smaller for the 3.8 license audit. `postcss.config.mjs` was required: Tailwind v4 does not compile `@theme` without `@tailwindcss/postcss`. |
...
| # | Component | File(s) | Owner | Status | Deps | Notes |
|---|---|---|---|---|---|---|
| 1.1 | **Leg A.** eCFR bulk XML parsed to citable sections | `pipeline/ecfr_parse.py`, `corpus/chunks/*.json` | **Tylin** | ⬜ | 0.9 | Title 47 Parts 5, 25, 97 and Title 15 Part 960 from `govinfo.gov/bulkdata/ECFR`. DIV5 = Part, DIV8 = Section. **Citation paths come from parsing the hardcoded paragraph labels inside the `P` elements**, not from element nesting. Ignore the `NODE` attribute. Pin each snapshot's `AMDDATE` and store it on every chunk. |
...
| 1.3 | **Leg A.** Embeddings + SQLite bundle frozen | `corpus/manifest.sqlite`, `corpus/vectors.npy` | **Tylin** | ⬜ | 1.1, 1.2 | Read-only bundle shipped with the app. Brute-force cosine in the route is fine at this size. |
...
| 1.5 | **Leg B.** Eval runner, local and CI | `eval/runner.py` | **Stephen** | ⬜ | 1.3, 1.4 | Bar: 90% or better with exact citations, all 6 traps abstaining. Rehearse on Ollama, verify on watsonx. |
...
| # | Component | File(s) | Owner | Status | Deps | Notes |
|---|---|---|---|---|---|---|
| Done 2026-08-22. Ten MissionInput fields, IndexedDB via lib/store.ts, nothing server-side. lvDeterminationDate has an explicit "not yet determined" state so the 97.207(g) dual clock fires correctly. Validation on submit, inline errors, focus moves to first invalid field.
Done 2026-08-24. @xyflow/react plus @dagrejs/dagre, rankdir TB. Dagre runs in useEffect gated on useNodesInitialized so layout happens after node measurement. Four verdicts each pair color with a text badge. Critical-path nodes get a border treatment, not a new color. Verified on the deployed URL.
2.3 ✂️ Cut 2026-08-25. Cut list item 4. vis-timeline is installed and imported nowhere. The graph already carries the demo and a second view of the same data is not worth the time before freeze. Stephen to pull vis-timeline from package.json.
...
2.9 → Done 2026-08-22. Computed on load, zero permissions. today and projectStart injected as props, never new Date() inside the component per D15. Both anchored to today, matching app/api/status/route.ts.
...
| 2.11 | Web push, secondary channel | `app/api/push/**`, `.github/workflows/deadline-check.yml` | **Tylin** | ⬜ | 2.10 | VAPID via `web-push`. **Subscriptions must be stored server-side (Vercel KV/Blob) or scheduled push is impossible.** GitHub Actions schedule with **offset minutes** (`4,14,24,34,44,54`), never `*/N` or `:00`, plus `workflow_dispatch` so it can be fired live in the demo. Delete on 404/410. |
| 2.12 | Capacitor 8 variant, static export build | `mobile/**`, `capacitor.config.ts`, `next.config.mobile.ts` | **Stephen** | ⬜ | **2.1, 2.2, 2.9 only** | **The mobile build deliberately does NOT wait for the full web UI.** It ships mission setup, the graph, the deadline banner, local notifications, native nav and offline. The Q&A panel (2.6), the citation panel (2.4) and push (2.11) are **not** in the first TestFlight build; they arrive in later builds of the same version, which do not need a fresh Beta App Review. This is what makes the Aug 22 target arithmetically possible at all. `output: 'export'`, `images.unoptimized: true`. **Never ship `server.url` in a store build.** No route handlers, no middleware, no request-time server components. Dynamic calls become client fetches against the hosted API from 1.17. |
| 2.13 | Local notifications, the flagship native capability | `mobile/notifications.ts` | **Stephen** | ⬜ | 2.12 | `@capacitor/local-notifications`, no push server needed on either platform. **iOS keeps only the soonest 64 pending requests**, so schedule the nearest 64 and reschedule on every app open. Android: `SCHEDULE_EXACT_ALARM` in the manifest, `checkExactNotificationSetting()` on launch, Android 13+ runtime permission. |
| 2.14 | Native navigation surface + working offline mode | `mobile/**` | **Stephen** | ⬜ | 2.12 | These are the documented Guideline 4.2 mitigations. Push alone is explicitly called out by Apple as insufficient. |
...
| 2.16 | Seed three real missions | `data/missions/*.json` | **Tylin** | ⬜ | 1.7 | **Moved off Stephen to rebalance Phase 2.** GT-1, DARLA-02, ASTRA-HyRAX. Real public data, every date labelled DOCUMENTED with source or ESTIMATED with basis. GT-1 is the headline mission and its source is live: `digitalcommons.usu.edu/smallsat/2021/all2021/21/`, DOI `10.26077/s4a1-qn29`. See "Real users". |
...
| # | Component | File(s) | Owner | Status | Deps | Notes |
|---|---|---|---|---|---|---|
| 3.1 | Eval wired into CI, PRs that regress citations do not merge | `.github/workflows/eval-gate.yml`, `eval/fixtures/**` | **Stephen** | ⬜ | 1.5 | **Separate workflow file, not Tylin's `ci.yml`.** **The backend question is decided here and it is not optional:** GitHub runners have no Ollama, and hitting watsonx on every PR needs repo secrets and burns the 300K monthly Lite cap. So **CI runs the eval against committed cached-response fixtures**, which tests retrieval, citation extraction and the abstention logic deterministically with no network and no key. The real watsonx score comes from a **manual run whose output is published to `docs/FACTS.json`** and dated. Say both numbers plainly in the README rather than implying the live score is gated. **The guard must FAIL under CI, never skip.** Assert the suite collected tests and hard-fail on any `N skipped`. |
| 3.2 | Eval MCP server exposed through IBM Context Forge | `eval/mcp_server.py`, `.bob/mcp.json` | **Stephen** | ⬜ | 1.5 | `pip install mcp-contextforge-gateway`, port 4444, JWT auth. Bridge via `mcpgateway.wrapper`. **No Docker on Apple Silicon**, there is no arm64 production image, and the daemon is not running anyway. |
|3.3 Done 2026-08-25. StatusPanel fetches /api/status client side. No hardcoded numbers, every figure from the payload. Citation gating keys on corpus_amddate. Models table renders so claimed versus invoked is checkable. Designed loading and error states, verify instructions stay visible if the fetch fails.
...
|3.5 Done 2026-08-25. docs/architecture.svg written in evidence-writer mode. Edge labels carry the invariant. Mermaid source handed to Stephen for the README.
| 3.6 | Wired-or-cut audit: grep the shipped code for every claim | `docs/claims-audit.md` | **Tylin** | ⬜ | 3.4 | For every named tool, model and integration, grep the source for its import or call. If absent, cut or reword. **Includes `.env.example`**, which is where plan-tier claims survive after the prose is fixed. |
| 3.7 | Second-model adversarial pass on the diff | n/a | **Stephen** | ⬜ | 3.6 | Codex via the companion runtime in **background** mode. Fixes written in response to a review carry fresh bugs that same-model review misses. |
| 3.8 | Security scan plus **license audit** | `.gitleaksignore`, `docs/THIRD_PARTY_NOTICES.md` | **Tylin** | ⬜ | n/a | gitleaks over **full history**, not the working tree. Every fingerprint hand-verified against its flagged commit before it goes in the ignore file. **Also runs the dependency license audit**, which the Definition of Done requires and no task owned: every direct and transitive dependency must be permissive. Specifically confirm no `elkjs` (EPL/GPL) and no `orbdetpy` (GPL-3.0) reached the tree, and record the ORBITM pinned commit and its MIT license. |
...
| 3.10 | Uptime watchdog on the deployed URL | `.github/workflows/uptime.yml` | **Tylin** | ⬜ | 1.17 | Offset minutes. **Check content, not status code**: a warm instance that lost its corpus still returns 200. |
...
| 4.3 | Deploy the frozen build, run the eval once more, publish the score | **Tylin** | ⬜ | Verify `.vercel/project.json` names `manifest-web` before `--prod`. |
...
| 4.11 | Learning activity complete for all three | **all three** | ⬜ | Submission requirement. |
