...
| 2.21 | Signed Android APK/AAB + Firebase App Distribution invite link | `android/**` | **Stephen** | ✅ 2026-08-24 (Claude): signed release APK verified (apksigner, CN=Stephen Sookra) and published durably at github.com/StephenSook/manifest/releases/tag/v1.0-beta.1 (GitHub Releases have no retention clock). Keystore machine-local at ~/.claude/keystores, keystore.properties gitignored, gradle signing wired. Firebase invite link pending only Stephen's `firebase login --reauth` (token expired) | 0.11, 2.12 | Free, invite links work without a Google group, 500 testers. **Do not attempt a Play Store listing**, the 12-tester 14-day closed-test rule makes it impossible inside the window. |
