...
| 4.12 | **Submit, Sat Aug 30** | **Stephen** | ⬜ | Project page on `aibuilderschallenge-bobhub.bemyapp.com` with team details, GitHub link, public video link. Verify the returned submission state, do not assume the click worked. Field-by-field check at submit: (1) the video field contains a public VIDEO URL, not the site URL and not pasted text (two rivals died exactly there); (2) all three SkillsBuild certificates are UPLOADED or linked in the certificate field, not merely held (rules: no cert, not judged; a rival with a live product missed this); (3) reload the page after save and re-read every field, the platform has no auto-save. |
...
- [x] **Q12 (Stephen, resolved 2026-08-26, Khadim backup):** Print `/api/status.runtime` on `/judge` and AskPanel so a judge sees the writer without curling. Claim-correcting after freeze: step 3 no longer calls the extractive eval a Granite pipeline, step 5 names stdio MCP not Context Forge, pending table names the real gaps. Video steal 2 (one seeded mission, no typing) parked in `docs/video/script.md` with recovered recording rules from prior cycles. Guard: `tests/test_judge_runtime_honesty.py`. Files: `app/judge/page.tsx`, `components/judge/StatusPanel.tsx`, `components/abstain/AskPanel.tsx` (Khadim lane, backup taken), `docs/video/script.md`, `docs/claims-audit.md`.
- [x] **Q13 (Stephen, resolved 2026-08-26, Khadim backup):** Steal 25. Tasks 1.13 and 2.22 close as honesty logs, not invented transcripts. Plan-mode session never captured. Lane enforcement is yaml + git + CI. `docs/bob-evidence/**` is Khadim's lane; backup taken. Commits `f50201d`, `5455420`, `051efd5`.
