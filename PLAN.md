...
| 0.5 | Four Bob skills, one per regulatory regime plus eval | `.bob/skills/*/SKILL.md` | **Tylin** | ✅ | 0.3 | `part-97-amateur`, `part-5-experimental`, `noaa-crsra`, `eval-bank`. Each with a `references/` folder. |
...
| 0.8 | Next.js 15 + TS strict + Tailwind v4 + shadcn new-york scaffold | `app/**`, `components/**`, `package.json` | **Khadim** | ✅ | 0.2 | Done 2026-08-22. Next.js 16.3.1, TS strict, Tailwind v4 theme tokens. Deps pinned: `@dagrejs/dagre` 1.1.4 (scoped). `vis-timeline` and TanStack Table were later removed (`3d18302`); timeline task 2.3 is cut. No `elkjs`. `test:engine` and `test:e2e` added. shadcn init deliberately NOT run: pages are hand-styled to the dense regulatory-instrument spec, shadcn would write competing CSS variables into `globals.css`, and skipping it keeps the tree smaller for the 3.8 license audit. `postcss.config.mjs` was required: Tailwind v4 does not compile `@theme` without `@tailwindcss/postcss`. |
...
| 0.13 | Verify watsonx: exact model IDs respond | `pipeline/scripts/watsonx_smoke.py` | **Tylin** | 🟡 | 0.9 | Confirm `ibm/granite-4-h-small`, `ibm/granite-guardian-3-8b`, `ibm/granite-embedding-278m-multilingual` all return 200. Record the region. |
| 0.14 | Dedicated Vercel project, verify link before any deploy | `.vercel/project.json` | **Khadim** | ✅ | 0.8 | Done 2026-08-22. Project `manifest-web` created under `khadim-drame-s-projects`, `.vercel/project.json` verified. Git auto-deploy deliberately NOT connected so deploys stay manual and the published URL is always one we chose. Vercel CLI on this machine is 59.x via npx, not the 53.2.0 in the toolchain table. |
...
| 1.13 | Bob evidence: Plan-mode output committed, screenshot #2 | `docs/bob-evidence/plan-mode-critical-path.md`, `docs/bob-evidence/bobalytics-02.png` | **Khadim** | ✅ 2026-08-26 Stephen backup | 0.3 | Both files exist. `bobalytics-02.png` is a subscription-usage screenshot, not a Plan session. A Plan-mode session for the build's critical path was never captured. The markdown is the dated refusal to invent one; it points at `engine/critical-path.ts` and `.bob/rules-plan/AGENTS.md`. Guard: `tests/test_bob_lane_enforcement.py`. |
| 1.14 | App shell proofs: React Flow with dagre, vis-timeline | `components/graph/*`, `components/timeline/*` | **Khadim** | ✅ Done 2026-08-24 for the graph half: dagre layout, no hydration warning, verified on the deployed URL. Timeline half pending 2.3. | 0.8 | **Client component only.** Gate on mounted or dynamic import, or SSR sizing breaks the layout. **Done means:** a hardcoded 5-node graph renders with dagre layout and no hydration warning in the console, and a 3-item timeline renders, both on the deployed URL from 1.17, not just locally. |
...
| 1.17 | **First production deploy** | Vercel | **Khadim** | ✅ | 0.8, 0.14 | Done 2026-08-22. Deployed at PASTE_REAL_URL_HERE. Shell only at deploy time. Unblocks 1.14, 3.10, and Stephen's `app/api/solar/route.ts` in 2.8. Redeploy is currently blocked: see Q9. |
| 1.18 | **Env and secrets provisioning, plus `.env.example`** | `.env.example`, Vercel project settings | **Khadim** | ⬜ | 0.14 | Remaining: `WATSONX_API_KEY`, `WATSONX_PROJECT_ID`, `WATSONX_REGION` in the Vercel project (Tylin supplies the values, 0.13). Optional `BLOB_READ_WRITE_TOKEN` for the corpus overlay. VAPID and KV are not needed: web push (2.11) is cut. `.env.example` already has empty WATSONX_* and Blob placeholders only. |
...
| # | Component | File(s) | Owner | Status | Deps | Notes |
|---|---|---|---|---|---|---|
| 2.1 | Mission setup flow, persists to IndexedDB only | `app/mission/**`, `lib/store.ts` | **Khadim** | ✅ | 1.7 | Done 2026-08-22. Ten MissionInput fields, IndexedDB via lib/store.ts, nothing server-side. lvDeterminationDate has an explicit "not yet determined" state so the 97.207(g) dual clock fires correctly. Validation on submit, inline errors, focus moves to first invalid field. |
| 2.2 | Dependency graph view, violated and at-risk visually distinct | `components/graph/**` | **Khadim** | ✅ | 1.7, 1.14 | Done 2026-08-24. @xyflow/react plus @dagrejs/dagre, rankdir TB. Dagre runs in useEffect gated on useNodesInitialized so layout happens after node measurement. Four verdicts each pair color with a text badge. Critical-path nodes get a border treatment, not a new color. Verified on the deployed URL. |
| 2.3 | Timeline view | `components/timeline/**` | **Khadim** | ✂️ Cut 2026-08-25. Cut list item 4. | 1.7 | vis-timeline was installed and imported nowhere. The graph already carries the demo and a second view of the same data is not worth the time before freeze. Khadim pulled vis-timeline and tanstack from package.json in 3d18302. |
...
| 2.5 | Abstention screen as a first-class state | `components/abstain/**` | **Khadim** | ✅ | 1.6 | Done 2026-08-25. AskPanel with four response states: abstained with no citations, abstained with retrieved sections, grounded and audited, and answered but unaudited. Abstention uses muted treatment, never --color-violated, because it is a designed state not an error. Three suggested questions including one trap so a judge sees a refusal without knowing what to type. |
...
| 2.7 | Deorbit compliance panel, the innovation surface | `components/deorbit/**` | **Khadim** | ✅ | 1.12 | Done 2026-08-24. Verdict, nominal lifetime against fccLimitYears, and the solar swing as the headline. Solar minimum maps to the longer lifetime and the failing verdict. Verified at 550 km Bc 180. Method rendered verbatim and labelled ESTIMATE per D4. No at-risk band invented: the five-year rule is binary. |
| 2.8 | **Surya inference wired** | `pipeline/surya_infer.py`, `app/api/solar/route.ts`, `data/surya-outlook.json` | **Stephen** | ✅ | 1.10, 1.11 | Done 2026-08-17. `pipeline/surya_infer.py` produces real output. HF repo: `nasa-ibm-ai4science/Surya-1.0` (Apache-2.0). Weights (1.8GB) + two bench frames (585MB each) cached. Weight interpolation (`_interpolate_state_dict_to_resolution`) maps 4096-trained pos_embed and spectral filters to 1024 resolution via bicubic/bilinear interp (standard ViT technique). Forward pass: `(1,13,2,1024,1024)` -> `(1,13,1024,1024)`. Activity index: AIA 94A top-1% proxy = 0.0231. `data/surya-outlook.json` written with real output. `app/api/solar/route.ts` shipped 2026-08-25 (commit 66c5d28) and is live on production: GET https://manifest-web-roan.vercel.app/api/solar returns 200 with f107_live, predicted_envelope, and surya_outlook. | `NASA-IMPACT/Surya` `easy_inference/run_easy_inference.py`, data from public S3 `nasa-surya-bench`, MPS or CPU, shipped `solar_flares_surya` checkpoint. **Surya's role must be stated before this starts, or the wired-or-cut audit cannot judge it and D1 cannot be applied.** Decided role: NOAA's predicted-flux envelope drives the lifetime bound in 1.11, and **Surya supplies a frozen near-term solar activity outlook shown beside that envelope**. No code applies the Surya index to the envelope or to the deorbit verdict. Input: SDO frames from the public bench bucket. Output: `SuryaOutlook` per the contract. Consumer: `app/api/solar/route.ts` and the 2.7 panel. **If Surya is absent, the verdict still computes from NOAA alone and the panel says so.** That is the honest framing and it is also the D7 fallback. **If by Aug 23 Surya is not producing a real output, cut it and say so in the README** rather than claiming it. |
| 2.9 | Deadline banner, primary alert, zero permissions | `components/deadline-banner/**` | **Khadim** | ✅ | 1.7 | Done 2026-08-22. Computed on load, zero permissions. today and projectStart injected as props, never new Date() inside the component per D15. Both anchored to today, matching app/api/status/route.ts. |
| 2.10 | PWA: manifest, Serwist service worker, offline corpus read | `app/manifest.ts`, `sw.ts` | **Khadim** | ✅ | 0.8 | Done 2026-08-24. Serwist 9.5.x `@serwist/next`, `app/manifest.ts` (name, short_name, start_url `/mission`, display standalone, theme_color, background_color, 512x512 icon plus apple-touch-icon), `sw.ts` wired in `next.config.ts`. Verified files exist on main. |
| 2.11 | Web push, secondary channel | `app/api/push/**`, `.github/workflows/deadline-check.yml` | **Tylin** | ✂️ Cut 2026-08-24. Cut list item 1. Confirmed 2026-08-26 freeze grep: no `app/api/push`, no `web-push` dependency, no `deadline-check.yml`. | 2.10 | Deadline banner (2.9) is the primary alert and needs zero permissions. Local notifications (2.13) cover mobile with no server. |
| 2.12 | Capacitor 8 variant, static export build | `mobile/**`, `capacitor.config.ts`, `next.config.mobile.ts` | **Stephen** | ✅ Done 2026-08-24 (Claude, commit 5ec2fbd): verified rendering on iPhone 17 sim, appId com.stephensookra.manifest, `scripts/build-mobile.sh` holds app/api aside during export | **2.1, 2.2, 2.9 only** | **The mobile build deliberately does NOT wait for the full web UI.** It ships mission setup, the graph, the deadline banner, local notifications, native nav and offline. The Q&A panel (2.6), the citation panel (2.4) and push (2.11) are **not** in the first TestFlight build; they arrive in later builds of the same version, which do not need a fresh Beta App Review. This is what makes the Aug 22 target arithmetically possible at all. `output: 'export'`, `images.unoptimized: true`. **Never ship `server.url` in a store build.** No route handlers, no middleware, no request-time server components. Dynamic calls become client fetches against the hosted API from 1.17. |
| 2.13 | Local notifications, the flagship native capability | `mobile/notifications.ts` | **Stephen** | ✅ Done 2026-08-24 (Claude, commit f13de15): 64-cap sliding window, resync on launch and resume, proven on sim (27 pending after permission grant) | 2.12 | `@capacitor/local-notifications`, no push server needed on either platform. **iOS keeps only the soonest 64 pending requests**, so schedule the nearest 64 and reschedule on every app open. Android: `SCHEDULE_EXACT_ALARM` in the manifest, `checkExactNotificationSetting()` on launch, Android 13+ runtime permission. |
| 2.14 | Native navigation surface + working offline mode | `mobile/**` | **Stephen** | ✅ Done 2026-08-24 (Claude, commit f13de15): native bottom tab bar, offline strip, Android hardware back, all native-only so the web build is unchanged | 2.12 | These are the documented Guideline 4.2 mitigations. Push alone is explicitly called out by Apple as insufficient. |
...
| 2.19 | **iOS build uploaded to App Store Connect** | n/a | **Stephen** | ✅ Done 2026-08-24 (Claude): build 1.0 (1) uploaded and VALID, app record "Manifest CubeSat Licensing" id 6804876175, PrivacyInfo.xcprivacy wired (commit dc4ae65), icon and splash landed (10c5977) | 2.12, 2.13, 2.14 | Bundle ID registered, `ITSAppUsesNonExemptEncryption` NO, `PrivacyInfo.xcprivacy` present (missing it means an ITMS-91053 upload rejection), icons and splash generated. Internal testing confirms it launches. |
| 2.20 | **First EXTERNAL TestFlight build submitted for Beta App Review** | n/a | **Stephen** | ✅ **APPROVED by Apple.** Verified 2026-08-25 via the ASC API (`GET /v1/builds/{id}/betaAppReviewSubmission`): app `Manifest CubeSat Licensing` id 6804876175, build 1, processingState VALID, expired false, betaReviewState **APPROVED**, submitted 2026-08-24T17:07 PT. Build is attached to the External Testers group. **PUBLIC LINK IS LIVE:** https://testflight.apple.com/join/huQrZpek , enabled 2026-08-25 on Stephen's explicit go-ahead via `PATCH /v1/betaGroups/212fade0-deda-4595-8039-805a57ab854b` with `publicLinkEnabled: true`, no tester limit. Verified from outside with no credentials: HTTP 200, no login redirect, page title `Join the Manifest CubeSat Licensing beta`. The URL now fills the iOS row in `docs/submission.md` and `README.md`. The Wednesday Aug 26 gate is fully MET: beta review passed AND the public link is live. | 2.19 | **Target Aug 22, no later than Aug 23.** Beta app description, contact info, and a reviewer note naming each native feature and exactly how to reach it. |
...
| 2.22 | Bobalytics screenshot #3, LANE-ENFORCEMENT transcript captured | `docs/bob-evidence/bobalytics-03.png`, `docs/bob-evidence/lane-enforcement.md` | **Khadim** | ✅ 2026-08-26 Stephen backup | n/a | **Bob 2.0.3 has no Orchestrator mode.** Replacement is lane enforcement, not a downgrade. `bobalytics-03.png` landed in `e5e451f`. `lane-enforcement.md` is the shipped `frontend` vs `evidence-writer` fileRegex, the `docs/architecture.svg` git event (`e6788e5`, `377a146`), and a CI table. Not a pasted Bob chat. Frontend refuses `docs/architecture.svg`; Evidence Writer allows it. Guard: `tests/test_bob_lane_enforcement.py`. PLAN had recorded a Frontend refusal on that path; the chat was never exported and was not invented. |

### Phase 3: Hardening, judge surfaces, review gate (Sun Aug 23 to Thu Aug 27)

| # | Component | File(s) | Owner | Status | Deps | Notes |
|---|---|---|---|---|---|---|
| 3.1 | Eval wired into CI, PRs that regress citations do not merge | `.github/workflows/eval-gate.yml`, `eval/fixtures/**` | **Stephen** | ✅ Done 2026-08-24 (Claude, commit b5e530c): offline fixtures mode on every push, ratchet floor 50 (raise-only), any trap answering fails, skipped rows fail. The 90 bar enforces at the Phase 4 freeze once watsonx lands | 1.5 | **Separate workflow file, not Tylin's `ci.yml`.** **The backend question is decided here and it is not optional:** GitHub runners have no Ollama, and hitting watsonx on every PR needs repo secrets and burns the 300K monthly Lite cap. So **CI runs the eval against committed cached-response fixtures**, which tests retrieval, citation extraction and the abstention logic deterministically with no network and no key. The real watsonx score comes from a **manual run whose output is published to `docs/FACTS.json`** and dated. Say both numbers plainly in the README rather than implying the live score is gated. **The guard must FAIL under CI, never skip.** Assert the suite collected tests and hard-fail on any `N skipped`. |
| 3.2 | Eval MCP server exposed through IBM Context Forge | `eval/mcp_server.py`, `.bob/mcp.json` | **Stephen** | 🟡 | 1.5 | Stdio MCP is wired: `eval/mcp_server.py` exposes `run_eval` and `eval_last_report`, `.bob/mcp.json` points Bob at it with empty env. Context Forge gateway registration is PARKED (opaque error on virtual-server create). README already says stdio, Bob-wired. Do not claim the gateway until it answers. **No Docker on Apple Silicon.** |
| 3.3 | `/judge` page: numbered three-minute itinerary with deep links | `app/judge/page.tsx` | **Khadim** | ✅ | 2.17 | Done 2026-08-25. StatusPanel fetches /api/status client side. No hardcoded numbers, every figure from the payload. Citation gating keys on corpus_amddate. Models table renders so claimed versus invoked is checkable. Designed loading and error states, verify instructions stay visible if the fetch fails. Remaining on this page: step 5 still says "the Orchestrator run transcript" (Khadim's lane, pinged 2026-08-26). |
| 3.4 | README to the required structure, plus `JUDGE.md` | `README.md`, `JUDGE.md` | **Stephen** | 🟡 | 2.18 | Partial 2026-08-17: Judge Quick Access table, judging criteria table, differentiator proof section. JUDGE.md created. Numbers section awaits FACTS.json (task 2.18). The platform requires exactly: Problem statement, Solution description, **AI approach and architecture**, Selected challenge theme, **How IBM Bob was used**. Plus the repo template sections. **The Bob section contains:** (1) a build-trace table from `git log` with a monotonically growing test count per commit and the reproduction command inline; (2) an explicit human/AI boundary statement (what Bob authored, what the humans decided); (3) an honest deduction if credits run out ("Bob as primary, not exclusive"); (4) an Evidence-to-Location table pointing at `.bob/custom_modes.yaml`, the skills, the Bobalytics screenshots and the lane-enforcement transcript. **The architecture section must describe only what `/api/status` self-reports**, never a path that is not running. `JUDGE.md` is a 90-second numbered walkthrough in the repo root, and it links to the `/judge` page rather than duplicating it. |
| 3.5 | Architecture diagram as a designed artifact | `docs/architecture.svg` | **Khadim** | ✅ | n/a | Done 2026-08-25. docs/architecture.svg written in evidence-writer mode. Edge labels carry the invariant. Node count is conditional on pathway (377a146). Mermaid source handed to Stephen for the README. |
| 3.6 | Wired-or-cut audit: grep the shipped code for every claim | `docs/claims-audit.md` | **Tylin** | ✅ | 3.4 | 2026-08-24 first pass plus mid-phase re-grep same day. **Freeze re-run 2026-08-26 (Stephen backup):** production live and extractive, vis-timeline/tanstack gone from package.json, pyatmos pin and gitleaks fingerprint and NOAA skill path already landed. Residual judge-page Orchestrator/Context Forge sentences and missing Bob transcripts are Khadim. watsonx still uncredentialed. |
| 3.7 | Second-model adversarial pass on the diff | n/a | **Stephen** | ✅ Done 2026-08-24 (Claude): TEN iterate-until-clean rounds via the Codex companion runtime. Rounds 1-3 (corpus reconstruction, cite-or-abstain, resync, baseline guard), rounds 4-9 (CFR resolver titles and fail-closed resolution, doc-matcher metadata, release signing fail-closed, provenance guard, citation spans, w-word boundaries, Part cues, unit-label ambiguity resolved by DELETING the carve-out: every CFR-shaped pathed reference is fail-closed). Round 10 CLEAN (1,022 direct assertions re-confirming all prior probes). Every finding verified against source before fixing; 21 fix commits 2712f97..13fb44b, all CI-green per SHA. Ask suite grew 13 to 44 tests | 3.6 | Codex via the companion runtime in **background** mode. Fixes written in response to a review carry fresh bugs that same-model review misses. |
| 3.8 | Security scan plus **license audit** | `.gitleaksignore`, `docs/THIRD_PARTY_NOTICES.md` | **Tylin** | ✅ | n/a | 2026-08-24: Gitleaks check-run on `d94d80e` concluded success (empty `.gitleaksignore`). Notices table now includes `@tailwindcss/postcss` and `@types/*` / `@vitejs/plugin-react`. Living CI license-guard fails if elkjs or orbdetpy appear in lockfiles. ORBITM not vendored. |
...
| 3.13 | Re-read the BeMyApp announcements feed | n/a | **Stephen** | ✅ | 0.15 | Final re-read 2026-08-26. Guest-reachable `aibuilderschallenge-bobhub.bemyapp.com/#/news/` still lists **6 articles**, newest still **July 31st 2026** (August GitHub Learning Lab / space-weather dashboard). Zero posts since the 2026-08-25 read. Public FAQ unchanged: auto-save still not implemented, video still 3:00 max, YouTube ID not URL for the illustration. No scoring-weight change on the public surfaces. Deadline still 31 August 11:59 PM. |
...
| 4.3 | Deploy the frozen build, run the eval once more, publish the score | **Tylin** | 🟡 | Deploy is Khadim's Vercel project and is live as of 2026-08-26. Stephen ran the URL eval twice on 2026-08-26 against https://manifest-web-roan.vercel.app: both 15/28 (53.6 percent), 6/6 traps, same ids as fixtures, recorded in `docs/FACTS.json` `eval_live` (second run 21:17 UTC). Remaining: Tylin 0.13 if the 90 bar is to be measured on watsonx. |
...
| 4.10 | Post-merge main CI watched to completion on the merged SHA | **Tylin** | ⬜ | Read `gh api repos/StephenSook/manifest/commits/<SHA>/check-runs` and require every conclusion to be `success`. Never trust a `--watch` exit code. |
...
## Bob setup: this is core to the build, not a chore
...
**Bobcoin budget: 40 each, 120 total.** A single non-trivial build can consume 40 on its own. Use Agent mode for routine edits. Reserve Plan runs and the write-scoped workspace modes for work that shows in the evidence trail. Meter at each phase boundary and log the count in Notes.
...
- [ ] **Q2 (Tylin, by Aug 17, and it is a budget not a lookup):** Do the exact watsonx Lite limits hold in our region? The 300,000 tokens per month and 2 requests per second figures come from secondary sources; IBM's own plan page could not be loaded. **Then compute the actual budget before 1.5 runs:** 34 eval items times (retrieval plus `ibm/granite-4-h-small` generation plus `ibm/granite-guardian-3-8b` audit) equals one eval run, so state in this cell how many tokens one run costs and therefore **how many watsonx eval runs the monthly cap allows**. That number decides whether the eval rehearses on Ollama by default (it should) and how many live verification runs we can afford. If the cap bites during judging week, flip that instance to Essentials rather than staying capped, and remember a free tier is metered in tokens, so a fully funded green app can still serve a dead model.
- [ ] **Q3 (Tylin, by Aug 18):** Does Docling's table extraction survive the FCC 26-47 appendix and the NASA-STD-8719.14C requirement tables? Owned by Tylin because he owns `pipeline/docling_ingest.py` and can act on the answer. Pass criterion: name the three specific tables and confirm each round-trips to the values printed on its source page. If one fails, extract that table by hand, cite the page, and mark it in the corpus.
...
- [ ] **Q6 (Tylin, before 1.3 ships):** Are `corpus/manifest.sqlite` and `corpus/vectors.f32` committed to git or built in CI? GitHub has a 100 MB per-file hard limit and Vercel has a serverless bundle limit. Measure the artifacts first, then decide, and record the decision in Shared Contracts.
...
- [x] **Q12 (Stephen, resolved 2026-08-26, Khadim backup):** Print `/api/status.runtime` on `/judge` and AskPanel so a judge sees the writer without curling. Claim-correcting after freeze: step 3 no longer calls the extractive eval a Granite pipeline, step 5 names stdio MCP not Context Forge, pending table names the real gaps. Video rule (one seeded mission, no typing) parked in `docs/video/script.md` with recovered recording rules from prior cycles. Guard: `tests/test_judge_runtime_honesty.py`. Files: `app/judge/page.tsx`, `components/judge/StatusPanel.tsx`, `components/abstain/AskPanel.tsx` (Khadim lane, backup taken), `docs/video/script.md`, `docs/claims-audit.md`.
- [x] **Q13 (Stephen, resolved 2026-08-26, Khadim backup):** Tasks 1.13 and 2.22 close as honesty logs, not invented transcripts. Plan-mode session never captured. Lane enforcement is yaml + git + CI. `docs/bob-evidence/**` is Khadim's lane; backup taken. Commits `f50201d`, `5455420`, `051efd5`.
...
| Bobcoin exhaustion | Treat 40 each as one day of heavy use, not a month's budget. Agent mode for routine edits, Plan runs reserved for evidence-worthy work, metered and logged at every phase boundary. If credits die early, the honest README line is "Bob as primary, not exclusive." |
...
| Someone ships the same idea | Other space-weather projects are likely. The differentiation is the legal verdict. Do not position against other projects, and never position against IBM's own tooling. |
...
**One commit per logical change.** Never bundle. The commit graph is the most honest record of how a build actually went, and atomic commits are how you get one that reads like engineering rather than like a dump.
...
5. **How IBM Bob built it, 2:35 to 3:00.** Fast montage over real screens: the three write-scoped custom modes, the four skills, a mode refusing an out-of-lane write, the eval running as an MCP tool through Context Forge, and the CI gate that blocks a PR which regresses citations.
...
_Last updated: 2026-08-15 by Stephen. Initial plan, restarted from day zero for a three-person team with sixteen days, adding the solar to deorbit-compliance spine._
