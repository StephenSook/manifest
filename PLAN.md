...
| 1.3 | **Leg A.** Embeddings + SQLite bundle frozen | `corpus/manifest.sqlite`, `corpus/vectors.f32` | **Tylin** | ✅ | 1.1, 1.2 | Local bundle uses hashing-trick-768 (Q2: Lite 300k tokens cannot embed 3524 chunks). Production path remains `ibm/granite-embedding-278m-multilingual` when WATSONX_* is set. Artifacts gitignored, uploaded by corpus-build.yml. schema.json records the model that actually ran. Blob upload blocked on Khadim 1.18 `BLOB_READ_WRITE_TOKEN`. |
...
| 3.6 | Wired-or-cut audit: grep the shipped code for every claim | `docs/claims-audit.md` | **Tylin** | ✅ | 3.4 | 2026-08-24 first pass. Re-run after 3.7 fixes and before freeze (4.1). ORBITM named in D4 but not imported. Context Forge not in tree. web-push cut. .env.example absent (Khadim 1.18). |
...
| 3.8 | Security scan plus **license audit** | `.gitleaksignore`, `docs/THIRD_PARTY_NOTICES.md` | **Tylin** | ✅ | n/a | 2026-08-24: `.gitleaksignore` empty (no fingerprints required ignore). THIRD_PARTY_NOTICES.md lists direct deps. elkjs absent. orbdetpy absent. ORBITM not vendored. |
...
| 3.10 | Uptime watchdog on the deployed URL | `.github/workflows/uptime.yml` | **Tylin** | ✅ | 1.17 | Workflow lands 2026-08-24. Content-checks `/api/status` for `deadline_violations_days`. Offset minutes `4,14,24,34,44,54`. **Fails if `vars.MANIFEST_DEPLOY_URL` is unset** (no skip). Goes green when Khadim 1.17 sets the variable. |
...
- [x] **Q6 (Tylin, resolved 2026-08-21, measured 2026-08-24):** Built in CI, stored in Vercel Blob. Local sizes after hashing freeze: sqlite ~3.5 MB, vectors.f32 ~10.8 MB (3524 x 768 float32). Under GitHub 100 MB, but binaries stay gitignored. Route fetches Blob on cold start, caches in memory. Recorded in Shared Contracts.
...
- [ ] **Q10 (Khadim, 2026-08-24 ping):** Task 1.18 must provision `WATSONX_API_KEY`, `WATSONX_PROJECT_ID`, `WATSONX_REGION=us-south`, `BLOB_READ_WRITE_TOKEN` on Vercel and as GitHub Actions secrets before `/api/ask` can run Granite+Guardian on the deployed URL and before corpus-build can upload. Task 1.17 must set repo variable `MANIFEST_DEPLOY_URL` or the uptime workflow fails honestly. Tylin's gather list (no values) is in `TYLIN_TASKS.md` under "External credentials to gather".
- [ ] **Q11 (Stephen, 2026-08-24 ping):** `data/missions/*.json` landed (task 2.16). `app/api/status/route.ts` still uses the inline GT-1 seed. Please switch the status route to read `data/missions/gt-1.json`. Also D4 names ORBITM; shipped decay.py uses pyatmos only (claims-audit 3.6).
...
_Last updated: 2026-08-24 by Tylin. Phase 1 leftovers closed (nested paragraphPath, hashing freeze, Q2/Q3/Q6). 2.6 extractive+Granite path. 2.11 cut. 2.16 seeds. 3.6/3.8/3.10 first pass._
