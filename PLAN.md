...
| 2.21 | Signed Android APK/AAB + Firebase App Distribution invite link | `android/**` | **Stephen** | ✅ COMPLETE 2026-08-25 (Claude). Signed release APK verified with apksigner (CN=Stephen Sookra) and published on GitHub Releases at tag `v1.0-beta.1`, which has no retention clock (AccessGate HR3). Firebase App Distribution now also live after Stephen reauthenticated: dedicated project `manifest-cubesat-2026`, Android app `1:72793499773:android:9f6194ec9e4523e366a914` (package com.stephensookra.manifest), release 1.0 (1) uploaded with reviewer notes and distributed to the `judges` group. A dedicated Firebase project was created rather than reusing the two existing `soar` projects, same reasoning as the Vercel dedicated-project rule. REMAINING, one console click: App Distribution has no CLI command for a PUBLIC invite link, so Stephen toggles it at console.firebase.google.com/project/manifest-cubesat-2026/appdistribution under Testers and Groups. Not blocking: the GitHub Release is the durable judge-facing download and needs no enrollment | 0.11, 2.12 | Free, invite links work without a Google group, 500 testers. **Do not attempt a Play Store listing**, the 12-tester 14-day closed-test rule makes it impossible inside the window. |
