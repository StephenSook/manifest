...
| 4.3 | Deploy the frozen build, run the eval once more, publish the score | **Tylin, but the deploy half is KHADIM** | 🟡 | **THERE IS NO GITHUB AUTO-DEPLOY. This row previously said the deploy could wait on "Khadim's link or a GitHub auto-deploy after the freeze is pushed", and the second option does not exist.** Measured 2026-08-25 23:5x ET: **36 commits pushed to `main` today** and production still serves a build older than the 10:35 commit (`/api/status` returns `scenarios: 0`, no `runtime` block, and `corpus_amddate: PENDING_CORPUS_FREEZE`, while `computed_at` updates per request, so the function is live and the CODE is stale). If Git integration were wired, 36 pushes would have redeployed it. Anyone planning around "it will auto-deploy when we push the freeze" is planning around a safety net that is not there. **Split the row.** The DEPLOY needs Khadim manually: there is no `manifest-web` project in Stephen's Vercel account, so only he can run `--prod`, and `.vercel/project.json` must name `manifest-web` before he does (a generic link clobbers another project's production). The EVAL RE-RUN AND SCORE PUBLICATION are independent of the deploy and Tylin can do them any time. **Action before Thursday, not on Thursday:** Khadim either connects Git integration so pushes deploy themselves, or confirms he can run `--prod` by hand. Either way it must be EXERCISED once while there is time to fix it. A deploy path that has not fired in 36 pushes is an assumption, not a path. |
