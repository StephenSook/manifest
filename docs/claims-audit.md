...
| watsonx generation, Guardian audit, granite embedding | app/api/ask/route.ts | **Revised 2026-08-31, the earlier row was stale in two ways.** Generation and the Guardian audit are WIRED AND NOW CREDENTIALED in production: `/api/status` returns `generation_backend: watsonx` and `guardian_audit: active`, on the basis of credential presence rather than a health check, which the payload says itself. The granite embedding is a different case and the old row got its reason wrong: it is not merely uncredentialed, it is STRUCTURALLY UNREACHABLE, because the committed corpus freeze determines the embedder and `app/api/ask/route.ts` therefore takes the hash branch in every deployment. `/api/status` reports `embedding_backend: hashing-trick-768`. `scripts/facts.py` carries the same explanation at the point where a stale attribution would otherwise be carried forward. Submission copy and README both state this now. |
